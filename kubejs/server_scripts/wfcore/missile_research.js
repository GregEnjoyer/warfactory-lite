...
// (CLAUDE.md "Research compute balance"): HV ~256 CWU/t, EV ~1024, IV ~4096. Runs in wfcore:research_unit.
...
//              Each run draws ~half the recipe over the 8 runs. Two substitutions are FORCED by the research
//              machine (it consumes concrete ItemStacks, not tags/NBT): the missiles' circuit TAGS become
//              concrete advanced_integrated_circuit (CIRC); charged lapotron becomes a plain crystal. The
//              fluid fuel and the lone fusee are dropped (fusee rounds out; fuel is a fluid, not an item).
...
ServerEvents.recipes(event => {
    ...
    // Item aliases mirror missiles.js. CIRC is the concrete stand-in for the missiles' circuit TAGS; LAPO is
    // the uncharged crystal (missiles use a charged one). The research machine can't take tags/NBT.
    const P    = 'gtceu:'
    ...
    const NODES = [
        ...
        // ── EMP branch (each advanced EMP its own node; circuit tags -> CIRC, lapotron uncharged) ──
